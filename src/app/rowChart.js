...
class RowChart extends React.Component {
    ...
    drawChart() {
        ...
        let processData = this.processData(this.props.newsData);
        var linearScale = d3.scaleLinear()
            .domain([0, 10000])
            .range([0, 100]);
        
        for (var i = 0; i < processData.length; i++) {
            processData[i].val = linearScale(processData[i].val);
        }
        debugger
        ...
        let y = d3.scaleBand()
            .range([0, height])
            .domain(processData.map((d) => d[this.state.yAxisAttribute]))
            .padding(.1);
        svg.append("g")
            .attr('class', 'axis y')
            .call(d3.axisLeft(y))
